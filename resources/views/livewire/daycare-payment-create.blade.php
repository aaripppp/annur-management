@php
    ...
    $cancelUrl = $cancelUrl ?? route('daycare.show', $child);
@endphp
<div>
    <div class="flex items-center gap-2 text-body-md text-on-surface-variant mb-3">
        ...
        <span>&rsaquo;</span>
        <a href="{{ route('daycare.show', $child) }}" wire:navigate class="hover:text-primary">{{ $child->nama_lengkap }}</a>
        <span>&rsaquo;</span>
        <span class="text-on-surface font-medium">{{ $isEdit ? 'Edit Pembayaran' : 'Input Pembayaran' }}</span>
    </div>
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-display-sm font-display-sm text-on-surface leading-tight">{{ $isEdit ? 'Edit Pembayaran Daycare' : 'Input Pembayaran Daycare' }}</h1>
            <p class="text-body-md text-on-surface-variant mt-0.5">{{ $isEdit ? 'Perbarui rincian transaksi tanpa mengubah nomor kwitansi.' : 'Catat beberapa pembayaran manual dalam satu transaksi.' }}</p>
            @if($isEdit)<p class="text-label-md font-semibold text-primary mt-1 font-numeric-data">{{ $receipt_number }}</p>@endif
        </div>
    </div>
        ...
        <div class="lg:col-span-2 flex flex-col gap-6">
            <section class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
                <div class="flex items-center justify-between gap-3 px-5 py-4 border-b border-outline-variant bg-surface-container-low">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            <h2 class="text-headline-sm font-headline-sm">Rincian Pembayaran</h2>
                            <p class="text-body-sm text-on-surface-variant mt-0.5">Jenis dan nominal diisi manual.</p>
                        </div>
                    </div>
                ...
                <div class="flex flex-col" data-testid="daycare-payment-items">
                    @error('items')
                        <div class="mx-5 mt-4 px-4 py-3 rounded-lg bg-error-container text-on-error-container text-body-sm">{{ $message }}</div>
                    @enderror
                    ...
                    <div class="divide-y divide-outline-variant/60 px-5">
                        @foreach($items as $index => $item)
                            <div wire:key="daycare-payment-item-{{ $itemKeys[$index] }}" data-testid="daycare-payment-item-row" class="grid grid-cols-[2rem_minmax(0,1fr)_2.5rem] md:grid-cols-[2.5rem_minmax(0,1fr)_14rem_2.5rem] gap-x-3 gap-y-2 items-start py-3.5">
                                ...
                                <div class="col-start-2 col-span-2 row-start-1 md:col-span-1 md:row-auto min-w-0">
                                    <label for="daycare-item-description-{{ $index }}" class="block md:hidden text-label-sm font-label-sm text-on-surface-variant mb-1">Jenis Pembayaran</label>
                                    <input id="daycare-item-description-{{ $index }}" type="text" wire:model.live.debounce.300ms="items.{{ $index }}.description" placeholder="Penitipan Bulan Agustus" class="w-full py-2.5 px-3 border-outline-variant focus:border-primary focus:ring-primary rounded-lg shadow-sm">
                                ...
                                <div class="col-start-2 row-start-2 md:col-start-3 md:row-auto min-w-0">
                                    <label for="daycare-item-amount-{{ $index }}" class="block md:hidden text-label-sm font-label-sm text-on-surface-variant mb-1">Nominal</label>
                                    <div class="relative" x-data="{
                                        ...
                                        <input id="daycare-item-amount-{{ $index }}" data-item-index="{{ $index }}" type="text" x-model="display" @input="updateAmount($event)" inputmode="numeric" placeholder="750.000" class="w-full pl-10 pr-3 py-2.5 border-outline-variant focus:border-primary focus:ring-primary rounded-lg shadow-sm font-numeric-data text-right">
                                    </div>
                                ...
                                <div class="col-start-3 row-start-2 md:col-start-4 md:row-auto flex justify-center pt-6 md:pt-1.5">
                                    @if(count($items) > 1)
                                        <button type="button" wire:click="removeItem({{ $index }})" class="p-1.5 rounded-lg text-on-surface-variant hover:text-error hover:bg-error/10" title="Hapus pembayaran">
                    ...
                    <div class="px-5 py-4 border-t border-outline-variant">
                        <button type="button" wire:click="addItem" class="w-full border border-dashed border-primary/50 text-primary hover:bg-primary-fixed/40 px-4 py-2.5 rounded-xl font-label-lg flex items-center justify-center gap-2">
            ...
            <section class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
                <div class="flex items-center gap-2.5 px-5 py-4 border-b border-outline-variant bg-surface-container-low">
                    ...
                    <h2 class="text-headline-sm font-headline-sm">Informasi Pembayaran</h2>
                </div>
                <div class="p-5 flex flex-col gap-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label for="daycare-bank" class="block text-label-md font-label-md text-on-surface mb-1">Metode / Rekening <span class="text-error">*</span></label>
                            <select id="daycare-bank" wire:model.live="bank_id" class="w-full py-2.5 px-3 border-outline-variant focus:border-primary focus:ring-primary rounded-lg shadow-sm">
                        ...
                        <div>
                            <label for="daycare-payment-date" class="block text-label-md font-label-md text-on-surface mb-1">Tanggal Pembayaran <span class="text-error">*</span></label>
                            <input id="daycare-payment-date" type="date" wire:model.live="payment_date" class="w-full py-2.5 px-3 border-outline-variant focus:border-primary focus:ring-primary rounded-lg shadow-sm">
                    ...
                    <div>
                        ...
                        <label class="border-2 border-dashed border-outline-variant rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-surface-container-low transition-colors cursor-pointer relative">
                            ...
                            @if($proof)
                                <p class="text-body-md font-semibold text-primary">{{ $proof->getClientOriginalName() }}</p>
                                <p class="text-body-sm text-on-surface-variant">File siap diunggah</p>
                            @elseif($isEdit && $existingProof)
                                <p class="text-body-md font-semibold text-primary">Bukti transfer sudah tersimpan</p>
                                <p class="text-body-sm text-on-surface-variant">Pilih file baru hanya jika ingin mengganti bukti.</p>
                            @else
                                <p class="text-body-md font-medium text-on-surface">Klik untuk memilih bukti pembayaran</p>
                                <p class="text-body-sm text-on-surface-variant mt-0.5">JPG, JPEG, PNG, atau PDF, maksimal 5 MB</p>
                            @endif
                        </label>
                    ...
                    <div>
                        <label for="daycare-notes" class="block text-label-md font-label-md text-on-surface mb-1">Catatan / Deskripsi <span class="text-on-surface-variant">(Opsional)</span></label>
                        <textarea id="daycare-notes" wire:model="notes" rows="3" placeholder="Catatan tambahan transaksi..." class="w-full py-2.5 px-3 border-outline-variant focus:border-primary focus:ring-primary rounded-lg shadow-sm"></textarea>
        ...
        <aside class="lg:col-span-1 lg:sticky lg:top-24">
            <div class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
                <div class="flex items-center gap-2.5 px-5 py-4 border-b border-outline-variant bg-surface-container-low">
                    ...
                    <h3 class="text-headline-sm font-headline-sm">Ringkasan Pembayaran</h3>
                </div>
                <div class="p-5 flex flex-col gap-5">
                    <div class="flex items-center gap-3 bg-surface-container-low p-3 rounded-xl border border-outline-variant">
                        <div class="w-10 h-10 rounded-full bg-primary-fixed text-on-primary-fixed flex items-center justify-center font-bold shrink-0">{{ strtoupper(substr($child->nama_lengkap, 0, 2)) }}</div>
                        <div class="min-w-0"><p class="font-bold text-on-surface truncate">{{ $child->nama_lengkap }}</p><p class="text-body-sm text-on-surface-variant">Kelas {{ $child->kelas }} &bull; Daycare</p></div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex flex-col">
                            @foreach($items as $index => $item)
                                <div wire:key="daycare-summary-item-{{ $itemKeys[$index] }}" class="flex items-center justify-between gap-3 py-2 border-b border-outline-variant/50 last:border-b-0 text-body-sm">
                                    <span class="min-w-0 truncate text-on-surface">{{ trim($item['description']) !== '' ? $item['description'] : 'Pembayaran '.($index + 1) }}</span>
                                    <span class="shrink-0 font-semibold text-on-surface whitespace-nowrap text-right font-numeric-data">Rp {{ $this->formatAmount($item['amount']) ?: '0' }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t border-outline-variant">
                        <div class="flex items-center justify-between gap-4 mb-3">
                            <span class="min-w-0 text-body-md font-bold text-on-surface">Total Pembayaran</span>
                            <span data-testid="daycare-total-amount" class="shrink-0 whitespace-nowrap text-right text-headline-md font-bold text-primary font-numeric-data">Rp {{ number_format($this->totalAmount(), 0, ',', '.') }}</span>
                        </div>
                    </div>
