<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-stack-lg">
        <div>
            ...
            <h1 class="text-display-sm font-display-sm text-on-surface">Import Daycare</h1>
            <p class="text-body-md text-on-surface-variant mt-1">Tambah anak daycare baru secara massal. Tidak memperbarui data yang sudah ada.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-6">
            <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden">
                <div class="px-6 py-4 border-b border-outline-variant">
                    <h2 class="text-headline-sm font-headline-sm text-on-surface">File Excel</h2>
                    <p class="text-body-sm text-on-surface-variant mt-1">Baris kosong akan diabaikan.</p>
                </div>
                <div class="p-6 space-y-5">
                    <div>
                        <label for="daycare-import-file" class="block text-label-md font-label-md text-on-surface mb-1">File Excel <span class="text-error">*</span></label>
                        <input id="daycare-import-file" type="file" wire:model="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" class="block w-full text-body-sm text-on-surface-variant file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:bg-primary-fixed file:text-primary file:font-label-md hover:file:bg-primary-fixed/80">
                        <p class="text-body-sm text-on-surface-variant mt-1">Format .xlsx, maksimum 12 MB.</p>
            ...
            <aside class="bg-surface-container-low border border-outline-variant rounded-2xl p-5 h-fit">
                ...
                <p class="text-body-sm text-on-surface-variant mt-2">Unduh template, isi nama lengkap dan kelas (wajib), lalu tambahkan biodata opsional lainnya.</p>
                ...
                <div class="mt-4 pt-4 border-t border-outline-variant text-body-sm text-on-surface-variant space-y-1">
                    ...
                    <p>Nama Panggilan &bull; Tempat Lahir</p>
                    <p>Tanggal Lahir (YYYY-MM-DD) &bull; Jenis Kelamin (L/P)</p>
                    <p>Alamat &bull; Nama Ayah &bull; No Telp Ayah</p>
                    <p>Nama Ibu &bull; No Telp Ibu</p>
                    <p class="pt-2">Kelas hanya boleh A, B, C, D, atau E.</p>
                    <p>Fitur ini hanya menambah anak baru, tidak memperbarui data yang sudah ada.</p>
                </div>
            </aside>
        </div>
    @elseif($step === 2)
        <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-outline-variant flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <h2 class="text-headline-sm font-headline-sm text-on-surface">Import Daycare - Preview</h2>
                    <p class="text-body-sm text-on-surface-variant mt-1">Periksa baris sebelum mengonfirmasi impor.</p>
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 px-6 py-4">
                @foreach([
                    ['value' => $preview['summary']['total'], 'label' => 'Total Baris', 'icon' => 'table_rows', 'icon_class' => 'bg-surface-container-high text-on-surface-variant'],
                    ['value' => $preview['summary']['new'], 'label' => 'NEW', 'icon' => 'person_add', 'icon_class' => 'bg-primary-fixed text-on-primary-fixed'],
                    ['value' => $preview['summary']['warning'], 'label' => 'WARNING', 'icon' => 'warning', 'icon_class' => 'bg-tertiary-container text-on-tertiary-container'],
                    ['value' => $preview['summary']['error'], 'label' => 'ERROR', 'icon' => 'error', 'icon_class' => 'bg-error-container text-on-error-container'],
                ] as $card)
                    ...
                    <tbody class="divide-y divide-outline-variant">
                        @foreach($preview['rows'] as $row)
                            @php
                                ...
                                $actionBadge = match($row['status']) {
                                    'error' => 'ERROR',
                                    default => 'NEW',
                                };
                                $actionClass = match($row['status']) {
                            ...
                            <tr class="{{ $row['status'] === 'error' ? 'bg-error-container/10' : '' }}">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex px-2 py-0.5 rounded-full text-label-sm font-label-sm leading-5 {{ $actionClass }}">{{ $actionBadge }}</span>
                                </td>
                                <td class="px-4 py-3 align-middle min-w-[280px]">
                                    <div class="flex flex-col items-start justify-center gap-1">
                                        @if($rowErrors !== [])
                                            <p class="text-body-sm leading-snug text-error">{{ implode(' ', $rowErrors) }}</p>
                                        @endif
                                        @if($rowWarnings !== [] && $row['status'] !== 'error')
                                            ...
                                            <p class="text-body-sm leading-snug text-on-surface-variant">{{ implode(' ', $rowWarnings) }}</p>
                                        @endif
                                    </div>
                                </td>
                            </tr>
    ...
    @else
        <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl p-8 text-center max-w-2xl mx-auto">
            ...
            <p class="text-title-lg text-on-surface mt-2">{{ $result['total'] }} anak daycare baru ditambahkan</p>

            <div class="grid grid-cols-1 gap-3 mt-6 text-left">
                <div class="bg-primary-container rounded-xl p-4">
                    <p class="text-headline-sm font-headline-sm text-on-primary-container">{{ $result['created'] }}</p>
                    <p class="text-body-sm text-on-primary-container">Anak Daycare Baru</p>
                </div>
            </div>
