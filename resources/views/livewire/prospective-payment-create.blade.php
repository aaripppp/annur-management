<div>
    ...
    <div class="flex items-center gap-2 text-body-md text-on-surface-variant mb-3">
        ...
        <span>&rsaquo;</span>
        <a href="{{ route('calon-siswa.show', $prospectiveStudent) }}" class="hover:text-primary transition-colors">Calon Siswa</a>
        <span>&rsaquo;</span>
        <span class="text-on-surface font-medium">Pembayaran Pendaftaran</span>
    </div>
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div class="min-w-0">
            <h1 class="text-display-sm font-display-sm text-on-surface leading-tight">Pembayaran Pendaftaran</h1>
            <p class="text-body-md text-on-surface-variant mt-0.5">Catat pembayaran biaya pendaftaran calon siswa. Setiap tagihan bisa dibayar penuh atau sebagian.</p>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 flex flex-col gap-6">
            ...
            <div class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
                <div class="flex items-center gap-2.5 px-5 py-4 border-b border-outline-variant bg-surface-container-low">
                    ...
                    <h2 class="text-headline-sm font-headline-sm text-on-surface">Data Calon Siswa</h2>
                </div>
                    ...
                    <div class="flex items-center justify-between gap-4 bg-primary-container/30 border border-primary/20 rounded-xl p-4">
                        <div class="flex items-center gap-3.5 min-w-0">
                            <div class="w-11 h-11 rounded-full bg-primary-fixed text-on-primary-fixed flex items-center justify-center font-bold text-body-lg shrink-0">
                                {{ strtoupper(substr($prospectiveStudent->nama_lengkap, 0, 2)) }}
                            </div>
                            <div class="min-w-0">
                                <div class="font-bold text-on-surface text-body-lg truncate">{{ $prospectiveStudent->nama_lengkap }}</div>
                                <div class="text-body-sm text-on-surface-variant flex items-center gap-2 mt-0.5">
                                    <span class="whitespace-nowrap font-numeric-data">{{ $prospectiveStudent->registration_number }}</span>
                                    <span>&bull;</span>
                                    <span class="truncate">Kelas Tujuan {{ $prospectiveStudent->schoolClass?->name ?? '—' }} &bull; TA {{ $prospectiveStudent->academicYear?->year ?? '—' }}</span>
                                </div>
                            </div>
                        </div>
                        <a href="{{ route('calon-siswa.show', $prospectiveStudent) }}" class="flex items-center gap-1.5 shrink-0 text-on-surface-variant border border-outline-variant rounded-lg px-2.5 py-1.5 hover:text-primary hover:bg-primary-fixed/40 transition-colors text-label-md" title="Kembali ke Detail Calon Siswa">
                ...
                <div class="p-4 flex flex-col gap-4">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse min-w-[640px]">
                                ...
                                <tbody class="divide-y divide-outline-variant/60">
                                    @foreach($outstandingBills as $index => $bill)
                                        @php
                                            ...
                                            $payAmount = $isSelected
                                                ...
                                                : 0;
                                        @endphp
                                        <tr wire:key="prospective-bill-row-{{ $bill['id'] }}" class="transition-colors {{ $isSelected ? 'bg-primary-fixed/30' : 'hover:bg-surface-container-low/60' }}">
                                            ...
                                            <td class="py-2.5 px-4">
                                                ...
                                                <span class="block text-body-sm text-on-surface-variant whitespace-nowrap">
                                                    Tahun Ajaran {{ $bill['academic_year'] ?? '—' }} &bull; Total tagihan Rp {{ number_format((float) $bill['amount'], 0, ',', '.') }}
                                                </span>
                                            </td>
                                            ...
                                            <td class="py-2.5 px-4 text-right whitespace-nowrap">
                                                @if($isSelected)
                                                    <div class="flex items-center justify-end" wire:key="prospective-amount-{{ $bill['id'] }}">
                    ...
                    @else
                        <div class="border border-dashed border-outline-variant rounded-xl px-6 py-10 text-center">
                            ...
                            <p class="text-body-sm text-on-surface-variant mt-1">Seluruh tagihan pendaftaran calon siswa telah lunas.</p>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="p-5 flex flex-col gap-5">
                    ...
                    <div class="flex items-center gap-3 bg-surface-container-low p-3 rounded-xl border border-outline-variant">
                        ...
                        <div class="overflow-hidden">
                            <div class="font-bold text-on-surface text-body-md truncate">{{ $prospectiveStudent->nama_lengkap }}</div>
                            <div class="text-body-sm text-on-surface-variant truncate font-numeric-data">{{ $prospectiveStudent->registration_number }}</div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex flex-col">
                            @forelse($selectedIds as $billId)
                                ...
                                @endphp
                                @if($selected)
                                    <div wire:key="prospective-summary-{{ $billId }}" class="flex justify-between items-center gap-3 text-body-md py-2 border-b border-outline-variant/50 last:border-b-0">
                                        <span class="min-w-0 truncate text-on-surface">{{ $selected['payment_type_name'] }}</span>
                    ...
                    <div class="flex flex-col gap-2.5 pt-2">
                        ...
                        <a href="{{ route('calon-siswa.show', $prospectiveStudent) }}" class="w-full py-2.5 text-center text-on-surface-variant font-label-lg border border-outline-variant rounded-xl hover:bg-surface-container transition-colors">
                            Batal
                        </a>
                    </div>
                </div>
