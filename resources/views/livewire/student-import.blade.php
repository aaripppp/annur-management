<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-stack-lg">
        <div>
            <a href="{{ route('siswa.index') }}" wire:navigate class="inline-flex items-center gap-1 text-label-md text-primary hover:underline mb-2">
                <span class="material-symbols-outlined text-[18px]">arrow_back</span>
                Data Siswa
            </a>
            <h1 class="text-display-sm font-display-sm text-on-surface">Import Siswa</h1>
            <p class="text-body-md text-on-surface-variant mt-1">Validasi seluruh data sebelum siswa, enrollment, dan tagihan dibuat.</p>
        </div>
        <div class="flex items-center gap-2 text-label-md text-on-surface-variant">
            @foreach([1 => 'Konteks', 2 => 'Preview', 3 => 'Selesai'] as $number => $label)
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-6">
            <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden">
                <div class="px-6 py-4 border-b border-outline-variant">
                    <h2 class="text-headline-sm font-headline-sm text-on-surface">Konteks Import</h2>
                    <p class="text-body-sm text-on-surface-variant mt-1">Tahun ajaran dan status berlaku untuk seluruh baris file.</p>
                </div>
                <div class="p-6 space-y-5">
                    <div>
                        <label for="import-academic-year" class="block text-label-md font-label-md text-on-surface mb-1">Tahun Ajaran <span class="text-error">*</span></label>
                    ...
                    <div>
                        <label class="block text-label-md font-label-md text-on-surface mb-2">Status / Konteks Siswa <span class="text-error">*</span></label>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label class="flex items-start gap-3 border rounded-xl p-4 cursor-pointer transition-colors {{ $importContext === 'prospective' ? 'border-primary bg-primary-fixed/40' : 'border-outline-variant hover:bg-surface-container-low' }}">
                                <input type="radio" name="import_context" value="prospective" wire:model.live="importContext" class="mt-1 text-primary focus:ring-primary border-outline-variant">
                                <span>
                                    <span class="block text-label-lg font-label-lg text-on-surface">Calon Siswa</span>
                                    <span class="block text-body-sm text-on-surface-variant mt-1">Tahun mendatang. Tagihan bulanan ditunda.</span>
                                </span>
                            </label>
                            <label class="flex items-start gap-3 border rounded-xl p-4 cursor-pointer transition-colors {{ $importContext === 'active' ? 'border-primary bg-primary-fixed/40' : 'border-outline-variant hover:bg-surface-container-low' }}">
                                <input type="radio" name="import_context" value="active" wire:model.live="importContext" class="mt-1 text-primary focus:ring-primary border-outline-variant">
                                <span>
                                    <span class="block text-label-lg font-label-lg text-on-surface">Siswa Aktif</span>
                                    <span class="block text-body-sm text-on-surface-variant mt-1">Hanya untuk tahun ajaran yang sedang aktif.</span>
                                </span>
                            </label>
                        </div>
                        @error('importContext') <span class="text-error text-body-sm mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label for="student-import-file" class="block text-label-md font-label-md text-on-surface mb-1">File Excel <span class="text-error">*</span></label>
                        <input id="student-import-file" type="file" wire:model="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" class="block w-full text-body-sm text-on-surface-variant file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:bg-primary-fixed file:text-primary file:font-label-md hover:file:bg-primary-fixed/80">
            ...
            <aside class="bg-surface-container-low border border-outline-variant rounded-2xl p-5 h-fit">
                ...
                <div class="mt-4 pt-4 border-t border-outline-variant text-body-sm text-on-surface-variant space-y-1">
                    ...
                    <p>NIS &bull; Nama Panggilan &bull; Jenis Kelamin</p>
                    <p>Tempat Lahir &bull; Tanggal Lahir (YYYY-MM-DD)</p>
                    <p>Nama Ayah &bull; No Telp Ayah &bull; Nama Ibu &bull; No Telp Ibu</p>
                    <p>Alamat &bull; Tanggal Masuk (YYYY-MM-DD)</p>
                    <p class="pt-2">File minimal cukup berisi kolom Nama Lengkap + Kelas.</p>
                    <p>Foto siswa tidak termasuk dalam import Excel.</p>
                </div>
            </aside>
        </div>
    @elseif($step === 2)
        <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-outline-variant flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <h2 class="text-headline-sm font-headline-sm text-on-surface">Import Siswa - Preview</h2>
                    <p class="text-body-sm text-on-surface-variant mt-1">{{ $preview['academic_year'] }} • {{ $preview['context'] }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 px-6 py-4">
                @foreach([
                    ...
                    ['value' => $preview['summary']['new'], 'label' => 'Siswa Baru', 'icon' => 'person_add', 'icon_class' => 'bg-primary-fixed text-on-primary-fixed'],
                ...
                <table class="w-full min-w-[900px] text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-outline-variant">
                        @foreach($preview['rows'] as $row)
                            @php
                                $isCrossLevel = $row['status'] === 'new' && str_starts_with($row['status_label'], 'Siswa Baru Jenjang');
                                $previousSchoolLevels = $row['previous_school_levels'] ?? [];
                                $statusBadge = match (true) {
                                    $row['status'] === 'error' => 'Bermasalah',
                                    $isCrossLevel => 'Siswa Baru • Alumni '.implode(' / ', $previousSchoolLevels),
                                    default => 'Siswa Baru',
                                };
                                $statusDetail = $row['status'] === 'error'
                                    ? preg_replace('/^Error:\s*/', '', $row['status_label'])
                                    : null;
                            ...
                            <tr class="{{ $row['status'] === 'error' ? 'bg-error-container/10' : '' }}">
                                ...
                                <td class="px-4 py-3 text-body-md font-label-md text-on-surface whitespace-nowrap">{{ $row['nis'] ?: '-' }}</td>
                                ...
                                <td class="px-4 py-3 align-middle min-w-[280px]">
                                    <div class="flex flex-col items-start justify-center gap-1">
                                        <div class="inline-flex items-center gap-1.5 flex-wrap">
                                            ...
                                            @if($rowWarnings !== [] && $row['status'] === 'new')
                                                <span class="inline-flex px-2 py-0.5 rounded-full text-label-sm font-label-sm leading-5 bg-surface-container-high text-on-surface-variant" title="{{ implode(' ', $rowWarnings) }}">
                                                    Perlu dicek admin
                                                </span>
                                            @endif
                                        </div>
                                        @if($row['status'] === 'error' && $statusDetail)
                                            <p class="text-body-sm leading-snug text-error">
                                                {{ $statusDetail }}
                                            </p>
                                        @endif
                                        @if($rowWarnings !== [] && $row['status'] !== 'error')
    ...
    @else
        <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl p-8 text-center max-w-2xl mx-auto">
            ...
            <p class="text-title-lg text-on-surface mt-2">{{ $result['total'] }} siswa diproses</p>
            <p class="text-body-md text-on-surface-variant mt-1">{{ $result['academic_year'] }} • {{ $result['context'] }}</p>
            ...
            <div class="grid grid-cols-1 gap-3 mt-6 text-left">
                <div class="bg-primary-container rounded-xl p-4">
                    ...
                    <p class="text-body-sm text-on-primary-container">Siswa Baru</p>
                </div>
            </div>
                ...
                <div class="mt-5 border border-outline-variant rounded-xl divide-y divide-outline-variant text-left">
                    @foreach($result['breakdown'] as $jenjang => $count)
                        <div class="flex justify-between px-4 py-2.5 text-body-md">
                            ...
                            <span class="font-label-md text-on-surface">{{ $count }} siswa</span>
                        </div>
                    @endforeach
                </div>
            @endif

            <a href="{{ route('siswa.index') }}" wire:navigate class="mt-6 inline-flex items-center justify-center bg-primary hover:bg-primary/90 text-on-primary px-6 py-2.5 rounded-xl font-label-lg transition-colors">
