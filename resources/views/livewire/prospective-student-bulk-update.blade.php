<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-stack-lg">
        <div>
            ...
            <h1 class="text-display-sm font-display-sm text-on-surface">Update Data Calon Siswa</h1>
            <p class="text-body-md text-on-surface-variant mt-1">Perbarui biodata calon siswa yang sudah ada. Tidak membuat calon siswa baru.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-6">
            <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden">
                <div class="px-6 py-4 border-b border-outline-variant">
                    <h2 class="text-headline-sm font-headline-sm text-on-surface">Filter Data Calon Siswa</h2>
                    <p class="text-body-sm text-on-surface-variant mt-1">Filter menentukan calon siswa yang muncul pada file yang akan diunduh.</p>
                </div>
                <div class="p-6 space-y-5">
                    <div>
                        <label for="bulk-update-academic-year" class="block text-label-md font-label-md text-on-surface mb-1">Tahun Ajaran Tujuan</label>
                        <select id="bulk-update-academic-year" wire:model="filterAcademicYearId" class="w-full border-outline-variant focus:border-primary focus:ring-primary rounded-lg shadow-sm">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="bulk-update-level" class="block text-label-md font-label-md text-on-surface mb-1">Jenjang</label>
                            <select id="bulk-update-level" wire:model.live="filterLevel" class="w-full border-outline-variant focus:border-primary focus:ring-primary rounded-lg shadow-sm">
                        ...
                        <div>
                            <label for="bulk-update-class" class="block text-label-md font-label-md text-on-surface mb-1">Kelas Tujuan</label>
                            <select id="bulk-update-class" wire:model="filterClassId" class="w-full border-outline-variant focus:border-primary focus:ring-primary rounded-lg shadow-sm" {{ $filterLevel === '' ? 'disabled' : '' }}>
                                <option value="">Semua Kelas</option>
                                @foreach($classes as $class)
                                    <option value="{{ $class->id }}">{{ $class->level_name }} - {{ $class->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <aside class="bg-surface-container-low border border-outline-variant rounded-2xl p-5 h-fit">
                ...
                <p class="text-body-sm text-on-surface-variant mt-2">Unduh file yang sudah berisi data calon siswa, lalu sesuaikan biodata pada kolom yang tersedia.</p>
                ...
                <div class="mt-4 pt-4 border-t border-outline-variant text-body-sm text-on-surface-variant space-y-1.5">
                    ...
                    <p class="flex items-start gap-2"><span class="material-symbols-outlined text-[18px] mt-0.5">info</span> No. Pendaftaran, Tahun Ajaran, dan Kelas hanya referensi.</p>
                    ...
                    <p class="flex items-start gap-2"><span class="material-symbols-outlined text-[18px] mt-0.5">close</span> Untuk mengosongkan nilai gunakan form Edit Calon Siswa.</p>
                </div>
            </aside>
        </div>
    @elseif($step === 2)
        <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-outline-variant flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <h2 class="text-headline-sm font-headline-sm text-on-surface">Update Data Calon Siswa - Preview</h2>
                    ...
                    <tbody class="divide-y divide-outline-variant">
                        @foreach($rows as $row)
                            <tr class="{{ $row['status'] === 'blocked' ? 'bg-error-container/10' : '' }}">
                                ...
                                <td class="px-4 py-3 text-body-md text-on-surface whitespace-nowrap">{{ $row['no_pendaftaran'] ?: '-' }}</td>
            ...
            <div class="px-6 py-4 border-t border-outline-variant bg-surface flex flex-col-reverse sm:flex-row justify-end gap-3 sticky bottom-0">
                ...
                <button type="button" wire:click="confirmUpdate" wire:loading.attr="disabled" @disabled($summary['ready'] === 0) class="bg-primary hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed text-on-primary px-6 py-2.5 rounded-xl font-label-lg transition-colors shadow-sm">
                    <span wire:loading.remove wire:target="confirmUpdate">Update {{ $summary['ready'] }} Calon Siswa</span>
    ...
    @else
        <section class="bg-surface-container-lowest border border-outline-variant rounded-2xl p-8 text-center max-w-2xl mx-auto">
            ...
            <p class="text-title-lg text-on-surface mt-2">{{ $result['updated'] }} calon siswa diperbarui</p>
            ...
            <div class="grid grid-cols-1 gap-3 mt-6 text-left">
                <div class="bg-primary-container rounded-xl p-4">
                    ...
                    <p class="text-body-sm text-on-primary-container">Biodata calon siswa diperbarui</p>
                </div>
            </div>
            ...
            <div class="mt-5 text-left text-body-sm text-on-surface-variant space-y-1.5 bg-surface-container-low rounded-xl p-4">
                ...
                <p>• ID calon siswa tidak berubah sehingga tagihan dan pembayaran pendaftaran tetap terhubung.</p>
                <p>• Kelas tujuan, tahun ajaran, dan status tidak terpengaruh oleh fitur ini.</p>
                <p>• Calon siswa yang sudah dikonversi tidak dapat diubah melalui fitur ini.</p>
            </div>
